...
void addItem(Account& acc, vector<Account> accounts) {
    string name, category; bool gotName = false;
    int price, quantity, refund = 0, count = 0;
    while(count != 5) {
        if(count == 4)
            if(cin.get() == '\n')
                break;
        ...
        if(choice == "price") {
            ...
            throw runtime_error("Unknown Choice");
        }
        count++;
    }
    if(gotName) {
    ...
    if(quantity < 0 || price < 0)
        throw runtime_error("Cannot Be Negetive");
    acc.addItem(Item::idCountItem, name, price, category, quantity, refund, acc.getUsername(), getCurrentDate());
...
vector<ItemIndex*> listItems(vector<Account> accounts, bool print) {
    ...
    if(isFilter()) {
        if(cin.get() != '\n')
            throw runtime_error("Id When Filter");
...
void buyItem(Account& acc, vector<ItemIndex*> itemIndexs, vector<Account>& accounts) {
    Item* item; int count, id; string command; string username; int whileCounter = 0;
    string city = "";
    ...
    Purchest* newPurchess = new Purchest(purchaseId, acc.getPerson(), item, count, postPrice, item->getPrice());
...
void purchaseCart(Account& loggedAccount, vector<Account>& accounts) {
    string city = ""; string choice;
    if(cin.get() != '\n') {
        cin >> choice;
        if(choice == "city")
            cin >> city;
